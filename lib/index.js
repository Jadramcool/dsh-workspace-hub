...
export function apply(ctx) {
    ...
    const costDisk = new Map();
    let costDiskPath = '';
    let costDiskDirty = false;
    let costDiskTimer = null;
    const tokenDisk = new Map();
    let tokenDiskPath = '';
    let tokenDiskDirty = false;
    let tokenDiskTimer = null;
    function loadCostDisk() {
        try {
    ...
    function scheduleSaveCostDisk() {
        costDiskDirty = true;
        if (costDiskTimer !== null || costDiskPath === '')
            return;
    ...
    function scheduleSaveTokenDisk() {
        tokenDiskDirty = true;
        if (tokenDiskTimer !== null || tokenDiskPath === '')
            return;
    ...
    async function sessionWatermark(id) {
        const s = svc();
        try {
            if (s.sessions && typeof s.sessions.get === 'function') {
                ...
                if (live && s.projections && typeof s.projections.snapshot === 'function') {
                    ...
                    if (snap && typeof snap.asOfSeq === 'number')
                        return snap.asOfSeq;
                }
            }
    ...
    function opencodeUsage(force = false) {
        const now = Date.now();
        if (!force && usageCache && now - usageCache.at < OPCODE_USAGE_TTL)
    ...
    function makeDiskStore(filename) {
        let data = null;
        let path = '';
        const filePath = () => {
            if (path !== '')
    ...
    const routes = [
        {
            ...
            handler: async (req, res) => {
                ...
                if (body && body.save === true) {
                    ...
                    if (data === null)
                        return writeJson(res, 400, { error: 'invalid data' });
