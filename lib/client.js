window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    .wsfm-diag{padding:3px 12px;font-size:10px;line-height:1.4;color:#e0af68;background:rgba(224,175,104,.08);border-bottom:1px solid rgba(224,175,104,.15);word-break:break-all}
    ...
    function Browser({ wide, expandSidebar, useSessions, useWorkspaces, workspacesApi, sessionsApi }) {
      ...
      React.useEffect(() => {
        ...
        setDiag({
          ...
          items: items.length,
          sessIds: items.reduce((n, ws) => n + (Array.isArray(ws.sessionIds) ? ws.sessionIds.length : -1), 0),
          need: needIds.length,
        })
        ...
        if (typeof window !== 'undefined') {
          timer = window.setTimeout(() => {
            hostCall('wsfm-tokens', { sessionIds: needIds }).then((res) => {
              ...
              setDiag((d) => d ? Object.assign({}, d, { got: Object.keys(res).length, nulls: Object.keys(res).filter((k) => res[k] === null).length, tok: Object.keys(next).length }) : d)
            }).catch((err) => { console.error('wsfm: token fetch failed', err); setDiag((d) => d ? Object.assign({}, d, { err: String(err && err.message ? err.message : err) }) : d) })
          }, 250)
        }
        return () => {
      ...
      return React.createElement('div', { className: 'wsfm-root' },
        ...
        diag
          ? React.createElement('div', {
              className: 'wsfm-diag',
              title: '调试信息：items=工作区数 sessIds=会话id总数 need=待请求 got=返回数 nulls=空值 tok=tokenMap键数 err=最近错误',
            },
            'D ' + diag.at + ' items=' + diag.items + ' ids=' + diag.sessIds + ' need=' + diag.need + (diag.got !== undefined ? ' got=' + diag.got : '') + (diag.nulls !== undefined ? ' null=' + diag.nulls : '') + (diag.tok !== undefined ? ' tok=' + diag.tok : '') + (diag.err ? ' ERR:' + diag.err : ''),
          )
          : null,
