window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Browser({ wide, expandSidebar, useSessions, useWorkspaces, workspacesApi, sessionsApi }) {
      ...
      const [goUsage, setGoUsage] = React.useState(null)
      ...
      // OpenCode Go plan usage (5h rolling / weekly / monthly), loaded once per panel open.
      React.useEffect(() => {
        if (!panelOpen || goUsage !== null || typeof fetch === 'undefined') return
        let alive = true
        hostCall('wsfm-usage', {}).then((res) => {
          if (!alive) return
          if (res && res.ok && res.usage) setGoUsage(res.usage)
          else setGoUsageErr(res && res.reason ? res.reason : 'unknown')
