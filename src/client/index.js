window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function hostCall(method, payload) {
      ...
      // Method names carry the legacy 'wsfm-' prefix (harness RPC era); the
      // HTTP routes are registered without it (/api/wsfm/tokens, /api/wsfm/cost).
      const path = method.indexOf('wsfm-') === 0 ? method.slice(5) : method
      return fetch('/api/wsfm/' + path, {
        method: 'POST',
    ...
    // Module-level guard: true once the host folder data has been hydrated into
    // the client. Before that, saveData() keeps localStorage-only so a slow
    // hydration cannot clobber the persisted copy with a premature edit.
    let foldersHydrated = false
    // Same guard for the model-price table persisted on the host.
    let pricesHydrated = false
    ...
    .wsfm-head{display:flex;align-items:center;justify-content:space-between;gap:6px 8px;padding:9px 12px 7px;border-bottom:1px solid var(--dsw-alias-border-l1,rgba(148,163,184,.12));flex-wrap:wrap}
    ...
    .wsfm-srch:focus-within{border-color:rgba(110,168,255,.5);background:var(--dsw-alias-bg-layer-1,rgba(148,163,184,.09));box-shadow:0 0 0 2px rgba(110,168,255,.07)}
    ...
    .wsfm-fcard{position:relative;background:var(--dsw-alias-bg-layer-1,rgba(148,163,184,.05));border:1px solid var(--dsw-alias-border-l1,rgba(148,163,184,.12));border-radius:10px;padding:3px;margin-bottom:8px;animation:wsfm-rise .28s ease both;transition:border-color .15s ease,background .15s ease;cursor:grab}
    ...
    .wsfm-fhead{position:relative;display:flex;align-items:center;gap:7px;padding:6px 8px;border-radius:8px;cursor:pointer;user-select:none;min-height:30px;border:1px dashed transparent;transition:background .12s ease}
    ...
    .wsfm-dot{width:8px;height:8px;border-radius:50%;flex:none;box-shadow:0 0 0 2px rgba(148,163,184,.12)}
    .wsfm-fname{font-weight:650;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12.5px}
    .wsfm-count{min-width:28px;box-sizing:border-box;text-align:center;font-size:10.5px;color:var(--dsw-alias-label-tertiary,#9aa3b2);background:var(--dsw-alias-interactive-bg-hover,rgba(148,163,184,.1));border-radius:99px;padding:2px 7px;flex:none;font-variant-numeric:tabular-nums}
    ...
    .wsfm-wlist{padding:2px 2px 4px 8px}
    ...
    .wsfm-whead{display:flex;align-items:center;gap:7px;padding:6px 8px;border-radius:8px;cursor:pointer;user-select:none;transition:background .12s ease}
    .wsfm-whead:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(148,163,184,.08))}
    .wsfm-wav{display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border-radius:6px;font-size:11px;font-weight:700;flex:none}
    .wsfm-wmid{flex:1;min-width:0}
    .wsfm-wtitle{font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12.5px;line-height:1.3}
    .wsfm-tokline{margin-top:2px;display:flex;align-items:center;line-height:1.3}
    ...
    .wsfm-slist{margin:3px 0 6px 16px;padding:4px 0 4px 10px}
    .wsfm-sess{position:relative;display:flex;flex-direction:column;gap:4px;padding:9px 11px;border-radius:8px;cursor:grab;margin:4px 0;transition:background .12s ease}
    .wsfm-sess.editing{flex-direction:row;align-items:center;gap:8px;padding:7px 9px}
    .wsfm-sess.static{cursor:default}
    ...
    .wsfm-sess-row1{display:flex;align-items:center;gap:7px;min-width:0;padding-right:52px}
    .wsfm-sess-row2{display:flex;align-items:center;gap:6px;padding-left:15px}
    .wsfm-sess-title{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px}
    .wsfm-sess-tok{font-size:10px;color:var(--dsw-alias-label-tertiary,#9aa3b2);font-variant-numeric:tabular-nums;white-space:nowrap;flex:none}
    .wsfm-sess-money{font-size:10px;color:var(--dsw-alias-state-success-primary,#4ade80);font-variant-numeric:tabular-nums;white-space:nowrap;flex:none;opacity:.9}
    ...
    .wsfm-st{width:8px;height:8px;border-radius:50%;flex:none;transition:transform .15s ease}
    ...
    .wsfm-gorefresh{margin-left:auto;padding:2px 9px;border:1px solid rgba(110,168,255,.25);border-radius:6px;background:var(--dsw-alias-button-primary-dimmed,rgba(110,168,255,.1));color:var(--dsw-alias-brand-primary,#6ea8ff);font-size:10px;font-family:inherit;font-weight:600;letter-spacing:.02em;text-transform:none;cursor:pointer;transition:background .15s ease,border-color .15s ease}
    .wsfm-fetherr{padding:5px 8px;margin-bottom:8px;border:1px solid rgba(251,191,36,.35);border-radius:7px;background:rgba(251,191,36,.08);color:#fbbf24;font-size:11px}
    .wsfm-gorefresh:hover{background:var(--dsw-alias-button-primary-dimmed,rgba(110,168,255,.2));border-color:rgba(110,168,255,.45)}
    .wsfm-gorefresh:active{transform:translateY(1px)}
    ...
    .wsfm-gorow{display:flex;align-items:center;gap:8px;padding:5px 6px;border-radius:7px}
    .wsfm-golabel{width:72px;flex:none;color:var(--dsw-alias-label-secondary,#b8bfcc);font-size:11px}
    .wsfm-gopct{width:38px;flex:none;text-align:right;font-size:11px;font-variant-numeric:tabular-nums;font-weight:600}
    .wsfm-goreset{flex:1;text-align:right;font-size:10px;color:var(--dsw-alias-label-tertiary,#9aa3b2)}
    ...
    .wsfm-price-grid input{width:100%;box-sizing:border-box;padding:5px 8px;border:1px solid transparent;border-radius:7px;background:var(--dsw-alias-bg-layer-1,rgba(148,163,184,.07));color:var(--dsw-alias-label-primary,#e7e9ee);font-size:12px;font-family:inherit;font-variant-numeric:tabular-nums;outline:none;text-align:center;transition:border-color .2s ease,box-shadow .2s ease,background .2s ease}
    .wsfm-price-grid input:hover{border-color:var(--dsw-alias-border-l1,rgba(148,163,184,.3))}
    .wsfm-price-grid input:focus{border-color:rgba(110,168,255,.5);background:var(--dsw-alias-bg-layer-1,rgba(148,163,184,.1));box-shadow:0 0 0 2px rgba(110,168,255,.07)}
    ...
    button:focus-visible,input:focus-visible,select:focus-visible{outline:1px solid rgba(110,168,255,.55);outline-offset:1px;border-radius:6px}
    `
    ...
    function saveData(data) {
      try {
        ...
        console.error('wsfm: local save failed', err)
      }
      // Persist to host disk (fire-and-forget; failure degrades to localStorage-only).
      // Skip until the host copy has been hydrated, so a slow hydration response
      // cannot be overwritten by an edit made before it arrived.
      if (typeof fetch !== 'undefined' && foldersHydrated) {
        hostCall('wsfm-folders', { save: true, data }).catch((err) => console.error('wsfm: folder persist failed', err))
      }
    }
    ...
    function savePrices(prices) {
      try {
        if (typeof localStorage !== 'undefined') localStorage.setItem(PRICE_KEY, JSON.stringify(prices))
      } catch (err) {
        console.error('wsfm: price save failed', err)
      }
      // Persist to host disk (fire-and-forget; failure degrades to localStorage-only).
      if (typeof fetch !== 'undefined' && pricesHydrated) {
        hostCall('wsfm-prices', { save: true, data: prices }).catch((err) => console.error('wsfm: price persist failed', err))
      }
    }
    ...
    // Persisted client caches: cost breakdowns and OpenCode Go usage are shown
    // instantly from localStorage, then refreshed in the background.
    const COST_CACHE_KEY = 'dsh.wsCost.v1'
    const USAGE_CACHE_KEY = 'dsh.wsUsage.v1'
    const USAGE_CACHE_TTL = 300000
    const loadCostCache = () => {
      try {
        const raw = window.localStorage.getItem(COST_CACHE_KEY)
        if (!raw) return {}
        const o = JSON.parse(raw)
        if (o && o.map && typeof o.map === 'object') return o.map
      } catch {}
      return {}
    }
    const saveCostCache = (map) => {
      try { window.localStorage.setItem(COST_CACHE_KEY, JSON.stringify({ at: Date.now(), map })) } catch {}
    }
    const loadUsageCache = () => {
      try {
        const raw = window.localStorage.getItem(USAGE_CACHE_KEY)
        if (!raw) return null
        const o = JSON.parse(raw)
        if (o && o.usage && Date.now() - o.at < USAGE_CACHE_TTL) return o.usage
      } catch {}
      return null
    }
    ...
    function Browser({ wide, expandSidebar, useSessions, useWorkspaces, workspacesApi, sessionsApi }) {
      ...
      const [pricesHydrated, setPricesHydrated] = React.useState(false)
      ...
      const [costMap, setCostMap] = React.useState(loadCostCache)
      ...
      const [goUsage, setGoUsage] = React.useState(loadUsageCache)
      const [goUsageErr, setGoUsageErr] = React.useState(null)
      ...
      const costRef = React.useState({ current: loadCostCache() })[0]
      ...
      // Background refresh of every workspace session's cost breakdown; the
      // cached values render immediately, this swaps in fresh numbers.
      React.useEffect(() => {
      ...
      // lookups are cheap (live projections / disk caches), so 30s is safe, and
      // polls are skipped while the tab is hidden to avoid pointless traffic.
      React.useEffect(() => {
        if (typeof fetch === 'undefined' || typeof window === 'undefined' || typeof document === 'undefined') return
        let alive = true
      ...
      React.useEffect(() => {
        if (typeof fetch === 'undefined') return
        let alive = true
        hostCall('wsfm-usage', {}).then((res) => {
          if (!alive) return
          if (res && res.ok && res.usage) {
            setGoUsage(res.usage)
            try { window.localStorage.setItem(USAGE_CACHE_KEY, JSON.stringify({ at: Date.now(), usage: res.usage })) } catch {}
          } else {
            setGoUsageErr(res && res.reason ? res.reason : 'unknown')
          }
        }).catch((err) => { if (alive) setGoUsageErr(String(err && err.message ? err.message : err)) })
        return () => { alive = false }
      }, [])

      // Manual refresh of OpenCode Go usage (bypasses the host's short cache).
      const refreshUsage = () => {
        if (typeof fetch === 'undefined') return
        setGoUsageErr(null)
        hostCall('wsfm-usage', { refresh: true }).then((res) => {
          if (res && res.ok && res.usage) {
            setGoUsage(res.usage)
            try { window.localStorage.setItem(USAGE_CACHE_KEY, JSON.stringify({ at: Date.now(), usage: res.usage })) } catch {}
          } else {
        ...
        }).catch((err) => setGoUsageErr(String(err && err.message ? err.message : err)))
      }

      // Clear any pending token-popup hide timer on unmount.
      React.useEffect(() => () => {
        if (tokTimerRef.current !== null && typeof window !== 'undefined') window.clearTimeout(tokTimerRef.current)
      }, [])
      ...
      // Hydrate folder data from host disk (source of truth) once on mount,
      // overriding the localStorage bootstrap so grouping follows the machine/DSH
      // profile rather than the browser.
      React.useEffect(() => {
        if (typeof fetch === 'undefined' || dataHydrated) return
        let alive = true
        hostCall('wsfm-folders', {}).then((res) => {
          if (!alive) return
          if (res && typeof res === 'object') {
            // Merge into the shape loadData() returns (host returns null when no
            // file has been written yet — then the localStorage bootstrap stands).
            const merged = loadData()
            merged.folders = Array.isArray(res.folders) ? res.folders : merged.folders
            merged.assign = res.assign && typeof res.assign === 'object' ? res.assign : merged.assign
            merged.order = res.order && typeof res.order === 'object' ? res.order : merged.order
            setData(merged)
          }
          foldersHydrated = true
          setDataHydrated(true)
        }).catch(() => { foldersHydrated = true; setDataHydrated(true) })
        return () => { alive = false }
      }, [dataHydrated])

      // Hydrate the model-price table from host disk on mount (user rows win);
      // official reference prices are always the baseline.
      React.useEffect(() => {
        if (typeof fetch === 'undefined' || pricesHydrated) return
        let alive = true
        hostCall('wsfm-prices', {}).then((res) => {
          if (!alive) return
          if (res && res.models && typeof res.models === 'object') {
            setPrices({ models: Object.assign({}, REF_PRICES, res.models) })
          }
          pricesHydrated = true
          setPricesHydrated(true)
        }).catch(() => { pricesHydrated = true; setPricesHydrated(true) })
        return () => { alive = false }
      }, [pricesHydrated])
      ...
      const renderTokenPanel = () => {
        ...
        if (panelView === 'prices') {
          ...
          const setPrice = (key, field, value) => {
            // Keep the raw string so typing decimals (e.g. "1." → "1.5") is not
            // truncated by an intermediate parseFloat; numbers are coerced at
            // calculation time. Only digits and one dot are allowed.
        ...
        const body = []
        if (fetchErr) body.push(React.createElement('div', { key: 'fetherr', className: 'wsfm-fetherr', title: '数据源暂时不可用，显示的是已缓存的值' },
          fetchErr === 'token' ? 'Token 数据获取失败，当前显示缓存值' : '费用数据获取失败，当前显示缓存值',
        ))
        ...
        body.push(React.createElement('div', { key: 'gosec', className: 'wsfm-psec' },
          'OpenCode Go 额度',
          React.createElement('button', { type: 'button', className: 'wsfm-gorefresh', title: '刷新额度（绕过缓存）', 'aria-label': '刷新额度', onClick: refreshUsage }, '刷新'),
        ))
        if (goUsageErr) {
          const hint = goUsageErr === 'no-key' ? '未找到 OpenCode 密钥（~/.local/share/opencode/auth.json）' : goUsageErr === 'network' ? '网络错误，无法连接 OpenCode 服务' : goUsageErr === 'http-401' ? '密钥无效或已过期' : goUsageErr === 'http-429' ? '请求过于频繁，稍后再试' : '查询失败：' + goUsageErr
          body.push(React.createElement('div', { key: 'goerr', className: 'wsfm-pempty' }, hint))
        } else if (!goUsage) {
          body.push(React.createElement('div', { key: 'goload', className: 'wsfm-pempty' }, '加载中…'))
        } else {
          const wins = [
            { key: 'rolling', label: '5 小时滚动' },
            { key: 'weekly', label: '本周' },
            { key: 'monthly', label: '本月' },
          ]
          let shown = 0
          wins.forEach((w) => {
            const win = goUsage[w.key]
            if (!win || win.percent === undefined) return
            shown++
            ...
            if (win.resetsAt) {
              ...
              if (!isNaN(d.getTime())) {
                resetTxt = (d.getMonth() + 1) + '月' + d.getDate() + '日 ' + String(d.getHours()).padStart(2, '0') + ':' + String(d.getMinutes()).padStart(2, '0') + ' 重置'
              }
            }
            body.push(React.createElement('div', { key: 'go' + w.key, className: 'wsfm-gorow' },
              React.createElement('span', { className: 'wsfm-golabel' }, w.label),
              React.createElement('span', { className: 'wsfm-pbar' },
                React.createElement('i', { style: { width: Math.max(pct > 0 ? 2 : 0, pct) + '%', background: color } }),
              ),
              React.createElement('span', { className: 'wsfm-gopct', style: { color } }, pct + '%'),
              resetTxt ? React.createElement('span', { className: 'wsfm-goreset' }, resetTxt) : null,
            ))
          })
          if (shown === 0) body.push(React.createElement('div', { key: 'goempty', className: 'wsfm-pempty' }, '暂无额度数据'))
        }
      ...
      const renderSessionRow = (s, wsId, sessList) => {
        ...
        // Drag-reorder only makes sense for sessions with a real workspace home;
        // search hits and orphaned (unassigned) sessions pass wsId=null and are
        // display-only — a null wsId would produce a fake 'session:null:…'
        // payload and an insert-line/drop mismatch.
        const dragable = wsId !== null && wsId !== undefined
        const stDot = React.createElement('span', { key: 'st', className: 'wsfm-st ' + statusOf(s) })
        const kids = []
        if (isEditing) {
          kids.push(stDot)
          ...
          kids.push(React.createElement('div', { key: 'row1', className: 'wsfm-sess-row1' },
            stDot,
            React.createElement('span', { className: 'wsfm-sess-title', title: s.displayTitle }, s.displayTitle),
            s.blank ? React.createElement('span', { key: 'tag', className: 'wsfm-sess-tag' }, '空白') : null,
          ))
          if (sTok) {
            kids.push(React.createElement('div', { key: 'row2', className: 'wsfm-sess-row2' },
              React.createElement('span', { className: 'wsfm-sess-tok', title: 'Token 消耗' }, fmtTokens(usageTotal(sTok))),
              sCost > 0 ? React.createElement('span', { key: 'money', className: 'wsfm-sess-money', title: '估算花费' }, fmtMoney(sCost)) : null,
            ))
          }
        ...
        return React.createElement('div', {
          key: s.id,
          className: 'wsfm-sess' + (isCurrent ? ' current' : '') + (dragable ? '' : ' static') + (isEditing ? ' editing' : ''),
          onClick: () => openSession(s.id),
          title: '打开会话',
          draggable: dragable,
          onDragStart: dragable ? (e) => {
            e.stopPropagation()
          ...
          onDragEnd: dragable ? resetDrag : undefined,
          onDragOver: dragable ? (e) => {
